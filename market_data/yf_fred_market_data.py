...
class MarketDataManager:
    ...
    def _fetch_remote_data(self,
        ...
        """
        Queries Yahoo Finance and FRED endpoints (Yields + CPI) for a
        ...
        fred_3m_url = fred_base_url + "?id=DGS3MO"
        fred_5y_url = fred_base_url + "?id=DGS5"
        fred_cpi_url = fred_base_url + "?id=CPIAUCSL"
        ...
        # Combine raw fetched daily levels (CPI will be monthly points
        # filled forward initially).
        remote_levels = pd.concat(
            [spx_series, yield_3m_series, yield_5y_series, cpi_series],
            axis=1)
        return remote_levels.loc[start_date:end_date]

    def load_or_update_market_levels(
            self, force_refresh: bool = False) -> pd.DataFrame:
        """
        Loads cached market levels or fetches incremental updates if local
        cache is stale.
        """
        ...
        if not self.cache_filepath.exists() or force_refresh:
            ...
            if self.auto_update and max_cached_date < (
                    today - pd.Timedelta(days=1)):
                save_file = True
                incremental_start_date = (
                        max_cached_date - pd.Timedelta(days=10))
                ...
                market_levels = market_levels[
                    ~market_levels.index.duplicated(keep="last")]
    ...
    def get_aligned_real_returns(
        ...
        """
        Resamples market levels to monthly frequency and computes 3D real
        monthly returns:
        [r_spx_real, r_3m_real, r_5y_real].
        """
        raw_levels = self.load_or_update_market_levels(
            force_refresh=force_refresh)

        # 1. Resample to Month-End frequency to align Daily Financial Data
        #    with Monthly CPI.
        monthly_levels = pd.DataFrame()
        monthly_levels["spx_close"] = (
                raw_levels["spx_close"].resample("ME").last())
        monthly_levels["cpi"] = (
                raw_levels["cpi"].resample("ME").last().ffill())
        monthly_levels["yield_3m"] = (
                raw_levels["yield_3m"].resample("ME").last())
        monthly_levels["yield_5y"] = (
                raw_levels["yield_5y"].resample("ME").last())
        monthly_levels = monthly_levels.dropna()

        # 2. Compute Nominal Monthly Returns
        # S&P 500 log returns
        spx_log_ret = (np.log(monthly_levels["spx_close"] /
                       monthly_levels["spx_close"].shift(1)))

        # CPI log inflation rate
        cpi_log_ret = (np.log(monthly_levels["cpi"] /
                       monthly_levels["cpi"].shift(1)))

        # 3M T-Bill nominal return (1/12th of previous month annualized yield)
        yield3m_diff = monthly_levels["yield_3m"].diff()

        # 5Y Note nominal return: Coupon yield -
        #                         (Modified Duration * Yield Change)
        # Assuming average modified duration D_5 ~ 4.5 years for
        # 5-year Treasuries
